...
/// 1ページ目：チーム全体成績のみ
/// 2ページ目：試合ごとの記録（得点・スタメン・メンバーチェンジ）
/// 3ページ目以降：選手ごとに1ページ（記録があるカテゴリのみ掲載。スパイク/サーブ/キャッチは総合のみ）
function printSelectedAggregate(teamName){
  ...
  showToast('印刷ダイアログで「用紙の向き：横」、詳細設定の「ヘッダーとフッター」をオフにしてください（URLの非表示に必要です）');
  window.print();
}
